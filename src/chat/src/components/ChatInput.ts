...
export default class ChatInput extends LitElement {
	...
	submit(e: KeyboardEvent) {
		if (this.value != "") {
			...
			if (this.replyId) {
				...
				if (!ev.canceled) {
					adapter.sendMessage(
						channel.channel_login.toLocaleLowerCase(),
						this.value
					);
				}
			}
	...
	async autocomplete() {
		if (this.value.length >= 1) {
			...
			if (sugs) {
				...
				if (cmd) {
					if (
						cmd.command.replace(this.commandCharacter, "") ==
							this.inputElement.innerText &&
						sugs.length > 1
					) {
						cmd = sugs[sugs.length - 2];
					}
					this.inputElement.innerText = cmd.command.replace(
	...
	handleKeyDown(e: KeyboardEvent) {
		...
		if (e.key == " " && false) {
			...
			if (channel) {
				const emote_map = message_parser.parseEmotes(
					lastWord,
					channel.channel_id,
					[]
				);
		...
		if (e.key == "Backspace") {
			if (
				this.inputElement.innerText.length == 0 ||
				this.inputElement.innerText == " " ||
				this.inputElement.innerText == "\n" ||
				this.inputElement.innerText == ""
			) {
	...
	getNameSugestions(str: string) {
		...
		if (channel) {
			...
			return chatters.filter((name) =>
				name.toLocaleLowerCase().match(str.toLocaleLowerCase())
			);
		}
		return [];
	}
		...
		return channel?.fetchCommandList((list_part: CommandList) => {
			if (list_part.commandPrefix == this.commandCharacter) {
				...
				for (let cmd of list_part.commands) {
					if (
						(cmd.command.match(str) || str.replace(" ", "") == "\n") &&
