...
const COMMAND_SERVICE = [StreamElementsApi, NightbotApi, TwitchCommands];
...
export default class Channel {
	...
	joinIRC() {
		...
		IRC.listen(IRCEvents.Joined, (msg: JoinMessage) => {
			...
			if (
				msg.channel_login == this.channel_login &&
				msg.user_login == acc.user_login
			) {
				this.onJoin(msg);
			}
		});
		...
		IRC.listen(IRCEvents.UserState, (msg) => {
			if (msg.channel === this.channel_login) {
				...
				this.moderator =
					msg.badges.find((b) => b.name == "moderator") !== undefined;
				this.broadcaster =
					msg.badges.find((b) => b.name == "broadcaster") !== undefined;
		...
		IRC.listen(IRCEvents.ChatState, (msg) => {
			if (msg.channel_login == this.channel_login) {
				...
				if (msg.follwers_only !== null) {
					this.follwers_only =
						msg.follwers_only !== "Disabled"
							? msg.follwers_only.Enabled.secs
							: -1;
		...
		IRC.listen(IRCEvents.ChatClear, (msg: ClearChatMessage) => {
			if (this.channel_login === msg.channel_login) {
				...
				const lines = this.chat.querySelectorAll(
				...
				if (msg.action.UserTimedOut) {
					...
					this.chat.appendNote(
						`${action.user_login} got timed out for ${Format.seconds(
							action.timeout_length.secs
						)}.`
					);
				}
			}
		...
		if (this.moderator || this.broadcaster) {
			if (this.follwers_only > -1) {
				...
				IRC.sendMessage(
					this.channel_login,
					this.channel_id,
					"/followers " + this.followermode_time
				);
			}
		}
	}
	...
	async connectPubsub() {
		...
		pubsub_features.listen([`polls.${this.channel_id}`]);
		...
		pubsub.onHypeTrain((data) => {
			if (data.channel_id == this.channel_id) {
				...
				this.chat.appendNote(
					`Hypetrain! Level ${data.level}. ${Format.seconds(detla)} left.`
				);
			}
		});
	}
